...
@login_required(login_url='/', redirect_field_name=None)          # funzione offerta da django per verificare che un utente abbia eseguito l'accesso prima di arrivare a questa view
def log_out(request):
...
def add_board(request):
    ...
    board_form = BoardCreationForm()            # non ci vanno i punti e virgola su pyrhon!x
...
def add_or_remove_user_to_board(request, board_id, user_id=''):
    ...
    if request.user in board_to_modify.owners.all():                # controllo per owner
        ...
        if request.method == 'POST':
            if request.POST.get('submit') == 'add_user_request':            # richiesta di aggiunta di un utente ad una board (collaboratore)
            ...
            elif request.POST.get('submit') == 'delete_user_request':       # richiesta di cancellazione di un utente da una board
                if user_id is not None:
            ...
            elif request.POST.get('submit') == 'add_user_to_admins_request':        # richiesta di aggiunta di un utente della board alla lista di amministratori della stessa (promozione ad admin)
                if user_id is not None:
            ...
            elif request.POST.get('submit') == 'remove_user_from_admins_request':           # richiesta di rimozione di un utente dalla lista degli amministratori (retocessione a collaboratore)
                if user_id is not None:
